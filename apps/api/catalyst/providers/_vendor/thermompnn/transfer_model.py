"""Vendored from Kuhlman-Lab/ThermoMPNN. DO NOT EDIT except for imports.

Upstream: https://github.com/Kuhlman-Lab/ThermoMPNN
Commit:   2b04fd370e399911b1fa5848112cc9013f084110
Licence:  MIT (see LICENSE beside this file)

Vendored because ThermoMPNN is not packaged. Pinned to an exact commit because a
moving `main` would silently change what a stored `weights_hash` refers to.

Local changes, kept to the minimum so the diff against upstream stays readable:
  * imports made relative, so this is importable as a package;
  * `from model_utils import featurize` removed - it is unused by the inference
    path and pulls in a training-time dependency chain (lightning, pandas).
"""
import torch
